...
export function VideoStatus({ 
  ...
  return (
    <div className={cn("flex items-center gap-2", className)}>
      ...
      {status === 'processing' && (
        <>
          <Loader2 className="h-4 w-4 animate-spin text-blue-500" data-testid="loader-icon" />
      ...
      {status === 'ready' && (
        <>
          <CheckCircle2 className="h-4 w-4 text-green-500" data-testid="check-icon" />
      ...
      {status === 'error' && (
        <>
          <AlertCircle className="h-4 w-4 text-destructive" data-testid="alert-icon" />
