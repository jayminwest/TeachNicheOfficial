...
export function useVideoUpload({
  ...
  const handleUploadSuccess = useCallback(async (uploadId: string) => {
    try {
      ...
      // Don't redirect automatically - let the form handle it
      // The asset ID is passed to onUploadComplete which the form will use
      console.log(`Video upload complete with asset ID ${assetId}, waiting for form submission`);
    } catch (error) {
